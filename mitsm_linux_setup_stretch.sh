...
#add-apt-repository 'deb [arch=amd64,i386] http://mirror.rackspeed.de/mariadb.org/repo/10.1/debian stretch main'
#
#echo "#MariaDB 10.1 repository list" >> /etc/apt/sources.list
#echo "deb [arch=amd64,i386] http://ftp.hosteurope.de/mirror/mariadb.org/repo/10.1/debian stretch main" >> /etc/apt/sources.list
#####################################################################################
...
apt-get install -y php7.0 php7.0-cli php7.0-curl php7.0-json php7.0-mysql php7.0-opcache apache2

# Install ssl-cert and create default snakeoil-cert
apt-get install -y ssl-cert
make-ssl-cert generate-default-snakeoil

tar cvfz /etc/apache2.tar.gz /etc/apache2

cp -f linux_config_script_files/daemon/apache2/000-default.conf /etc/apache2/sites-available/000-default.conf
...
# start ignoring errors since we print errors now
set +e
